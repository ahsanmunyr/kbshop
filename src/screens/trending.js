// @ts-nocheck
import { View, Text, StatusBar, SafeAreaView, Platform, StyleSheet, Button, TouchableOpacity, TouchableWithoutFeedback, Image, FlatList, ScrollView, ActivityIndicator, AppState,Dimensions } from 'react-native'
...
import ShopModal from './home/Live/ChildComponent/ShopModal';
import ChatModal from './home/Live/ChildComponent/ChatModal';
import ShareModal from './home/Live/ChildComponent/ShareModal';
...
function RenderPost({
  ...
  const [controlShare, onChangeControlShare] = useState(false)
      ...
      <>
        <TouchableOpacity
          ...
          {(pause && (item._id == current._id)) ? (
            <View style={{
              ...styles.videoPlay,
              top: (item?.orientation == undefined || item?.orientation != 'portrait') && above ? (flatLayout.height / 2) - responsiveWidth((15 + 56.25)) : (flatLayout.height / 2) - responsiveWidth(tablet? 2: 15 / 2)
            }}>
    ...
    <View key={index} style={{
...

      {
        ...
        <ShopModal
          ...
          setControlShare={onChangeControlShare}
          hid={hidden}
...
function LiveCycle({ likeGet, like, route, getPreviousEvents, previousEvents, getPerviousEventItem, getPreviousEventsItem, getProducts, authRed, kbslyShortURL, createChatToken, navigation, createChatTokenClear, setIsBio }) {
  ...
  useEffect(() => {
    setLoading(true)
    getPreviousEvents(1, [],null,null,"likes").then(() => setLoading(false))
  }, [])
    ...
    return (
      <>
        {(previousEvents.total_records) > previousEvents.data.length ? (
          <View style={{ ...styles.footer, height: flatLayout.width }}>
  ...
  const loadMoreRecorded = useCallback(() => {
    ...
    if (previousEvents.total_records > previousEvents.data.length) {
      ...
      getPreviousEvents(recordedPage, [], null, "loadMore","likes").then(() => {
    ...
    return (
      ...
      <View style={{
        ...
        <StatusBar
          ...
          showHideTransition={'fade'}
        />
        <View style={{ flex: 1, width: responsiveWidth(100), justifyContent: 'center', }}>
          <FlatList
...
const styles = StyleSheet.create({
  ...
  videoPlay: {
    ...
    // position: 'absolute',
    // top: 3,
    top: responsiveFontSize(tablet ? 45 : 45.5),
    position: 'absolute',
    zIndex: 99,
    // right: 3,
  },
...
function mapStateToProps({ upcomingEvents, liveEvents, previousEvents, authRed, getPerviousEventItem, hostEventLiveRed }) {
  return { upcomingEvents, liveEvents, previousEvents, authRed, getPerviousEventItem, hostEventLiveRed }
}
